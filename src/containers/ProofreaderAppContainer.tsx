  ...
  const handleProofread = useCallback(async () => {
    if (!essayText.trim()) {
      toast({
        ...
        description: "Type or paste your essay before running proofread.",
        status: "info",
        duration: 4000,
        isClosable: true,
      });
    ...
    try {
      ...
      toast({
        ...
        description: nextIssues.length
          ? `We found ${nextIssues.length} ${nextIssues.length === 1 ? "suggestion" : "suggestions"}.`
      ...
      try {
        ...
        if (!range) {
          toast({
            ...
            description: "We could not locate the original phrasing to replace.",
            status: "info",
            duration: 3000,
            isClosable: true,
          });
    ...
    <AppLayout
      commandPanel={
        <CommandPanel
          ...
          helperText={
            lastRunAt &&
            `Last analyzed ${lastRunAt.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`
          }
        />
      }
      editor={
