...
const normalizeIssue = (issue: RawIssue, index: number): ProofreadIssue | null => {
...
const normalizeResponse = (payload: unknown): ProofreadIssue[] => {
...
const generateMockIssues = (text: string): ProofreadIssue[] => {
  ...
  if (doubleSpaceRange) {
    results.push({
      ...
      suggestion: " ",
      description: "Two spaces found together. Replace with a single space for readability.",
      start: doubleSpaceRange.start,
      end: doubleSpaceRange.end,
    });
  }
  ...
  if (veryIndex !== -1) {
    results.push({
      ...
      description: "Repeated modifier detected. Consider a stronger single word.",
      start: veryIndex,
      end: veryIndex + "very very".length,
    });
  }

  const sentenceWithoutPeriod = /\b([A-Z][^.!?]{20,})(?=$|\s{2,})/m.exec(text);
  if (sentenceWithoutPeriod) {
    ...
    if (range) {
      results.push({
        ...
        description: "Sentence appears to be missing terminal punctuation.",
        start: range.start,
        end: range.end,
      });
    }
...
export const requestProofreading = async (text: string): Promise<ProofreadIssue[]> => {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Proofreading request failed with status ${response.status}`);
    }
